...
const sendConfirmationEmail =  async (options) =>
{
    ...
    await sendMail(options.email, 'Private GP Appointment Confirmation - Medical Express Clinic' , content, event);
...
const sendRefundNotificationEmail =  async (options) =>
{
    ...
    await sendMail(options.email, 'Refund Deposit Notification (Private GP) - Medical Express Clinic' , content, null);
}
