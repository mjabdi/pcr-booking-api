...
const {
  ScreeningBooking,
} = require("./../../models/screening/ScreeningBooking");
...
router.get("/getinvoicereports", async function (req, res, next) {
  try {
    const invoiceReports = await GlobalParams.findOne({
      name: "invoiceReports",
    });
  ...
  try {
    ...
    for (var index = 0; index < invoicesArray.length; index++) {
      ...
      resultMap[dateStr] = {
        ...
        pcr:
          (resultMap[dateStr] ? resultMap[dateStr].pcr || 0 : 0) +
          (clinic === "pcr" ? fee : 0),
        gynae:
          (resultMap[dateStr] ? resultMap[dateStr].gynae || 0 : 0) +
          (clinic === "gynae" ? fee : 0),
        gp:
          (resultMap[dateStr] ? resultMap[dateStr].gp || 0 : 0) +
          (clinic === "gp" ? fee : 0),
        paediatrician:
          (resultMap[dateStr] ? resultMap[dateStr].paediatrician || 0 : 0) +
          (clinic === "paediatrician" ? fee : 0),
        std:
          (resultMap[dateStr] ? resultMap[dateStr].std || 0 : 0) +
          (clinic === "std" ? fee : 0),
        blood:
          (resultMap[dateStr] ? resultMap[dateStr].blood || 0 : 0) +
          (clinic === "blood" ? fee : 0),
        screening:
          (resultMap[dateStr] ? resultMap[dateStr].screening || 0 : 0) +
          (clinic === "screening" ? fee : 0),
        corporate:
          (resultMap[dateStr] ? resultMap[dateStr].corporate || 0 : 0) +
          (clinic === "corporate" ? fee : 0),
        derma:
          (resultMap[dateStr] ? resultMap[dateStr].derma || 0 : 0) +
          (clinic === "derma" ? fee : 0),
      };
...
router.post("/searchallinvoicesbydate", async function (req, res, next) {
  try {
    ...
    const invoices = [...invoicesArray].sort(
    ...
    res
      .status(200)
      .send({ status: "OK", count: result.length, result: result });
  } catch (err) {
    console.log(err);
    res
      .status(500)
      .send({ status: "FAILED", count: 0, result: [], error: err });
  }
});
...
router.post("/updateinvoice", async function (req, res, next) {
  try {
    ...
    if (!invoice) {
      res
        .status(400)
        .send({ status: "FAILED", error: "INVALID-INVOICE-NUMBER" });
      return;
    }
...
router.get("/getallbloodcodes", async function (req, res, next) {
  try {
    const result = await BloodCode.find({ hidden: { $ne: true } })
      .sort({ code: 1 })
      .exec();
...
router.post("/updatecode", async function (req, res, next) {
  try {
    ...
    if (newCode.isBloodTable) {
      await BloodCode.updateOne(
        { _id: id },
        {
          description: newCode.description,
          price: newCode.price,
          hidden: newCode.hidden ? true : false,
        }
      );
    } else {
      await MedexCode.updateOne(
        { _id: id },
        { description: newCode.description, price: newCode.price }
      );
    }
...
async function generateNewInvoiceNumber() {
  let lastInvoiceNumber = await GlobalParams.findOne({
