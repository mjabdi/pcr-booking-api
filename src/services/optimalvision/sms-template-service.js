...
const { GetPreviewSMS, SendManualSMS } = require('./sms-template-manager');
const ObjectId = mongoose.Types.ObjectId;
const { OVBooking } = require('../../models/optimalvision/OVBooking');
const { Patient } = require('../../models/optimalvision/Patient');

router.post('/getsmspreview', async function(req, res, next) {
    ...
    try{
        ...
        const preview = await GetPreviewSMS(templateId,booking,patient)
...
router.post('/sendmanualsms', async function(req, res, next) {
    ...
    try{
        ...
        await SendManualSMS(templateId, sendTo, booking, patient)
...
router.post('/registernewtemplate', async function(req, res, next) {
    ...
    try{
        ...
        const found = await SMSTemplate.findOne({templateID: template.templateID})                                       
...
router.post('/updatetemplate', async function(req, res, next) {
    ...
    try{
        ...
        await SMSTemplate.updateOne({_id : req.body.id}, {...req.body.template});
...
router.post('/deletetemplate', async function(req, res, next) {
    ...
    try{
        await SMSTemplate.deleteOne({_id : req.query.id});
        res.status(200).send({status: 'OK'});
...
router.get('/gettemplatebyid', async function(req, res, next) {
    ...
    try{
         req.query.id = ObjectId(req.query.id);
         const result = await SMSTemplate.findOne({_id : req.query.id});
...
router.get('/gettemplatebytemplateid', async function(req, res, next) {
    ...
    try{
         ...
         const result = await SMSTemplate.findOne({templateID : templateID});
         res.status(200).send(result);
    }
    catch(err)
    {
...
router.get('/getalltemplates', async function(req, res, next) {
    ...
    try{
        ...
        const result = await SMSTemplate.find().sort({timeStamp: -1}).exec();
         res.status(200).send(result);
    }
    catch(err)
    {
