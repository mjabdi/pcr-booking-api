...
const getHolidays = async () => {
  const offDays = await OffDays.find({
    $or: [{ service: "gynae" }, { service: "clinic" }],
  });
...
const getDefaultTimeSlots = async (date) => {
  ...
  const result = await WorkingHours.findOne({ service: "gynae" });

  const startingHour = result?.startingHour || 11;
  const endingHour = result?.endingHour || 15;
