...
router.get("/getshouldrefundscount", async function (req, res, next) {
  try {
    const count = await DermaBooking.countDocuments({
      deleted: { $eq: true },
      deposit: { $gt: 0 },
    });
...
router.get("/getbookingscountbydatestr", async function (req, res, next) {
  try {
    ...
    const result = await DermaBooking.countDocuments({
      bookingDate: dateStr,
      deleted: { $ne: true },
      status: "booked",
    }).exec();
...
router.get("/getbookingsbydatestrandtime", async function (req, res, next) {
  try {
    ...
    const result = await DermaBooking.find({
      bookingDate: dateStr,
      bookingTime: timeStr,
      deleted: { $ne: true },
      status: "booked",
...
router.get("/getallbookings", async function (req, res, next) {
  try {
    ...
    const result = await DermaBooking.find({ deleted: { $ne: true } })
...
router.get("/getdeletedbookings", async function (req, res, next) {
  try {
    const limit = parseInt(req.query.limit) || DEFAULT_LIMIT;
    const result = await DermaBooking.find({ deleted: { $eq: true } })
      .sort({ bookingDate: -1, bookingTimeNormalized: -1 })
      .limit(limit)
      .exec();
    res.status(200).send(result);
...
router.post("/addnewbooking", async function (req, res, next) {
  try {
    ...
    let { fullname, bookingDate, bookingTime, phone, email, notes, service } =
      req.body;
...
router.post("/bookappointment", async function (req, res, next) {
  ...
  try {
    const found = await DermaBooking.findOne({
    ...
    await sendAdminNotificationEmail(
      NOTIFY_TYPE.NOTIFY_TYPE_DERMATOLOGY_BOOKED,
      booking
    );
