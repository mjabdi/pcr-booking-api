...
router.post('/submitformdata' , async function (req,res,next) {
    ...
    try{
        ...
        await DermaBooking.updateOne({_id: bookingId} , {formData: JSON.stringify(formData)});
        res.status(200).send({status : "OK"});
    }
    catch(err)
    {
...
router.post('/paybooking' , async function (req,res,next) {
    ...
    try{
        ...
        await DermaBooking.updateOne({_id: bookingId} , {paid: true, OTCCharges:price, paidBy: paidBy, corporate: corporate ? corporate : ''});
        res.status(200).send({status : "OK"});
    }
    catch(err)
    {
...
router.post('/unpaybooking' , async function (req,res,next) {
    ...
    try{
        const bookingId = ObjectId(req.query.id);
        await DermaBooking.updateOne({_id: bookingId} , {paid: false, OTCCharges:0, paidBy: '', corporate: ''});
        res.status(200).send({status : "OK"});
    }
    catch(err)
    {
...
router.get('/getshouldrefundscount', async function(req, res, next) {
    try{
        const count = await DermaBooking.countDocuments({deleted : {$eq: true}, deposit: {$gt : 0} })
...
router.get('/getbookingscountbydatestr', async function(req, res, next) {
    ...
    try{
         ...
         const result = await DermaBooking.countDocuments({bookingDate: dateStr , deleted : {$ne : true }, status: 'booked'}).exec();
...
router.get('/getallbookingscountall', async function(req, res, next) {
    ...
    try{
         const result = await DermaBooking.countDocuments({deleted : {$ne : true }}).exec();
         res.status(200).send({status: "OK", count : result});
    }
    catch(err)
    {
...
router.get('/getallbookingscountbydatestr', async function(req, res, next) {
    ...
    try{
         ...
         const result = await DermaBooking.countDocuments({bookingDate: dateStr , deleted : {$ne : true }}).exec();
         res.status(200).send({status: "OK", count : result});
    }
    catch(err)
    {
...
router.get('/getbookingsstatsbydatestr', async function(req, res, next) {
    ...
    try{
         ...
         if (!dateStr || dateStr.length <= 0)
         {
          ...
          const result = await DermaBooking.aggregate([
            {
                $match: {
                  bookingDate: dateStr,
                  deleted: {$ne : true}
...
router.get('/getbookingscountbydatestrandtime', async function(req, res, next) {
    ...
    try{
         ...
         const result = await DermaBooking.countDocuments({bookingDate: dateStr , bookingTime: timeStr, deleted : {$ne : true }, status: 'booked'}).exec();
         res.status(200).send({status: "OK", count : result});
    }
    catch(err)
    {
...
router.get('/getallbookingscountbydatestrandtime', async function(req, res, next) {
    ...
    try{
         ...
         const result = await DermaBooking.countDocuments({bookingDate: dateStr , bookingTime: timeStr, deleted : {$ne : true }}).exec();
         res.status(200).send({status: "OK", count : result});
    }
    catch(err)
    {
...
router.get('/getbookingsbydatestrandtime', async function(req, res, next) {
    ...
    try{
        ...
        const result = await DermaBooking.find({bookingDate: dateStr , bookingTime: timeStr, deleted : {$ne : true }, status: 'booked'}).sort({timeStamp:1}).exec();
...
router.get('/getallbookingsbydatestrandtime', async function(req, res, next) {
    ...
    try{
        ...
        const result = await DermaBooking.find({bookingDate: dateStr , bookingTime: timeStr, deleted : {$ne : true }}).sort({timeStamp:1}).exec();
        res.status(200).send({status: "OK", bookings : result});
   }
   catch(err)
   {
...
router.get('/getallbookings', async function(req, res, next) {
    ...
    try{
         ...
         const result = await DermaBooking.find( {deleted : {$ne : true }} ).sort({bookingDate: -1, bookingTimeNormalized: -1}).limit(limit).exec();
...
router.get('/getdeletedbookings', async function(req, res, next) {
    ...
    try{
        const limit = parseInt(req.query.limit) || DEFAULT_LIMIT
         const result = await DermaBooking.find( {deleted : {$eq : true }} ).sort({bookingDate: -1, bookingTimeNormalized: -1}).limit(limit).exec();
         res.status(200).send(result);
    }
    catch(err)
    {
...
router.get('/gettodaybookings', async function(req, res, next) {
    ...
    try{
        const today = dateformat(new Date(), 'yyyy-mm-dd');
         ...
         const result = await DermaBooking.find({bookingDate : today, deleted : {$ne : true }}).sort({bookingTimeNormalized: 1}).exec();
         res.status(200).send(result);
    }
    catch(err)
    {
...
router.get('/getoldbookings', async function(req, res, next) {
    ...
    try{
        const today = dateformat(new Date(), 'yyyy-mm-dd');
        const limit = parseInt(req.query.limit) || DEFAULT_LIMIT
         const result = await DermaBooking.find({bookingDate : {$lt : today}, deleted : {$ne : true }}).sort({bookingDate: -1, bookingTimeNormalized: -1}).limit(limit).exec();
         res.status(200).send(result);
    }
    catch(err)
    {
...
router.get('/getfuturebookings', async function(req, res, next) {
    ...
    try{
        const today = dateformat(new Date(), 'yyyy-mm-dd');
        const limit = parseInt(req.query.limit) || DEFAULT_LIMIT
         const result = await DermaBooking.find({bookingDate : {$gt : today}, deleted : {$ne : true }}).sort({bookingDate: 1, bookingTimeNormalized: 1}).limit(limit).exec();
         res.status(200).send(result);
    }
    catch(err)
    {
...
router.get('/getrecentbookings', async function(req, res, next) {
    ...
    try{
         const result = await DermaBooking.find({deleted : {$ne : true }}).sort({timeStamp: -1}).limit(10).exec();
         res.status(200).send(result);
    }
    catch(err)
    {
...
router.get('/getrecentbookingsall', async function(req, res, next) {
    ...
    try{
        const limit = parseInt(req.query.limit) || DEFAULT_LIMIT
         const result = await DermaBooking.find({deleted : {$ne : true }}).sort({timeStamp: -1}).limit(limit).exec();
         res.status(200).send(result);
    }
    catch(err)
    {
...
router.get('/getbookingsbyref', async function(req, res, next) {
    ...
    try{
         const result = await DermaBooking.find({bookingRef : req.query.ref});
         res.status(200).send(result);
    }
    catch(err)
    {
...
router.get('/getbookingbyid', async function(req, res, next) {
    ...
    try{
        req.query.id = ObjectId(req.query.id);
         const result = await DermaBooking.findOne({_id : req.query.id});
         res.status(200).send(result);
    }
    catch(err)
    {
...
router.post('/addnewbooking', async function(req, res, next) {
    try{
        ...
        let {fullname, bookingDate, bookingTime, phone, email, notes, service} = req.body
        ...
        const payload =  {fullname, bookingDate, bookingTime, phone, email, notes, package}


        const booking = new DermaBooking(
            {
                ...payload,
                bookingRef: ref,
...
router.post('/bookappointment', async function(req, res, next) {
    ...
    try{
        ...
        const found = await DermaBooking.findOne({email : req.body.email,
        ...
        await sendAdminNotificationEmail(NOTIFY_TYPE.NOTIFY_TYPE_DERMATOLOGY_BOOKED,booking)
...
router.post('/updatebookappointment', async function(req, res, next) {
    ...
    try{
        ...
        const oldBooking = await DermaBooking.findOne({_id : req.body.bookingId});

        await DermaBooking.updateOne({_id : req.body.bookingId}, {...req.body.person});

        const newBooking = await DermaBooking.findOne({_id : req.body.bookingId});
...
router.post('/updatebookappointmenttime', async function(req, res, next) {
    ...
    try{
        ...
        await DermaBooking.updateOne({_id : req.body.bookingId}, {bookingDate : req.body.bookingDate, bookingTime : req.body.bookingTime, bookingTimeNormalized: NormalizeTime(req.body.bookingTime)});

        const booking = await DermaBooking.findOne({_id : req.body.bookingId});
...
router.post('/changetopatientattended', async function(req, res, next) {
    ...
    try{
        ...
        await DermaBooking.updateOne({_id : req.query.id}, {status : 'patient_attended'});
...
router.post('/deletebookappointment', async function(req, res, next) {
    ...
    try{
        ...
        await DermaBooking.updateOne({_id : req.query.id}, {deleted : true});
...
router.post('/undeletebookappointment', async function(req, res, next) {
    ...
    try{
        ...
        await DermaBooking.updateOne({_id : req.query.id}, {deleted : false});
