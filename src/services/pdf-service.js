...
router.get('/downloadscreeningregform', async function(req, res, next) {
    ...
    try{
        ...
        const pdfBuffer = await createPDFForScreeningRegistration(id);        
        
        await ScreeningBooking.updateOne({_id: id, status:'booked'}, {status: 'patient_attended'});
