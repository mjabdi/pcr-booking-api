...
const sendConfirmationEmail =  async (options) =>
{
    ...
    content += `<p>Thank you for booking your appointment at the Medical Express Clinic. We look forward to welcoming you.</p>`;
    ...
    const targetForm = `https://londonmedicalclinic.co.uk/medicalexpressclinic/user/form/corporate/${options._id}`;
    const target = `https://londonmedicalclinic.co.uk/medicalexpressclinic/user/edit/corporate/${options._id}`;
    ...
    content += `<li> Package : ${options.service.toUpperCase()} </li>`;
    ...
    await sendMail(options.email, 'Appointment Confirmation - Medical Express Clinic' , content, event);
...
const sendRegFormEmail =  async (options) =>
{
    ...
    await sendMail(options.email, 'Registration Form for Private GP Visit - Medical Express Clinic' , content, null);
}
