...
router.post("/setclinicnotes", async function (req, res, next) {
  try {
    ...
    await PaediatricianBooking.updateOne({ _id: bookingId }, { clinicNotes: notes });
    res.status(200).send({ status: "OK" });
  } catch (err) {
    console.log(err);
    res.status(500).send({ status: "FAILED", error: err.message });
...
router.get("/getshouldrefundscount", async function (req, res, next) {
  try {
    const count = await PaediatricianBooking.countDocuments({
      deleted: { $eq: true },
      deposit: { $gt: 0 },
    });
...
router.get("/getbookingscountbydatestr", async function (req, res, next) {
  try {
    ...
    const result = await PaediatricianBooking.countDocuments({
      bookingDate: dateStr,
      deleted: { $ne: true },
      status: "booked",
    }).exec();
...
router.get("/getbookingsstatsbydatestr", async function (req, res, next) {
  try {
    ...
    const result = await PaediatricianBooking.aggregate([
      {
        $match: {
          bookingDate: dateStr,
          deleted: { $ne: true },
...
router.get("/getbookingsbydatestrandtime", async function (req, res, next) {
  try {
    ...
    const result = await PaediatricianBooking.find({
      bookingDate: dateStr,
      bookingTime: timeStr,
      deleted: { $ne: true },
      status: "booked",
...
router.get("/getallbookings", async function (req, res, next) {
  try {
    ...
    const result = await PaediatricianBooking.find({ deleted: { $ne: true } })
...
router.get("/getdeletedbookings", async function (req, res, next) {
  try {
    const limit = parseInt(req.query.limit) || DEFAULT_LIMIT;
    const result = await PaediatricianBooking.find({ deleted: { $eq: true } })
      .sort({ bookingDate: -1, bookingTimeNormalized: -1 })
      .limit(limit)
      .exec();
    res.status(200).send(result);
...
router.get("/getbookingsbyref", async function (req, res, next) {
  try {
    const result = await PaediatricianBooking.find({ bookingRef: req.query.ref });
    res.status(200).send(result);
  } catch (err) {
    res.status(500).send({ status: "FAILED", error: err.message });
  }
...
router.get("/getbookingbyid", async function (req, res, next) {
  try {
    ...
    const result = await PaediatricianBooking.findOne({ _id: req.query.id });
    res.status(200).send(result);
  } catch (err) {
    res.status(500).send({ status: "FAILED", error: err.message });
  }
...
router.post("/addnewbooking", async function (req, res, next) {
  try {
    ...
    let { fullname, bookingDate, bookingTime, phone, email, notes, smsPush } =
      req.body;
    ...
    const booking = new Paediatrician({
      ...payload,
      bookingRef: ref,
...
router.post("/updatebookappointment", async function (req, res, next) {
  ...
  try {
    const oldBooking = await PaediatricianBooking.findOne({ _id: req.body.bookingId });
    ...
    const newBooking = await PaediatricianBooking.findOne({ _id: req.body.bookingId });
...
router.post("/updatebookappointmenttime", async function (req, res, next) {
  ...
  try {
    ...
    const booking = await PaediatricianBooking.findOne({ _id: req.body.bookingId });
...
router.post("/changebacktobookingmade", async function (req, res, next) {
  ...
  try {
    await PaediatricianBooking.updateOne({ _id: req.query.id }, { status: "booked" });
...
router.post("/deletebookappointment", async function (req, res, next) {
  ...
  try {
    await PaediatricianBooking.updateOne({ _id: req.query.id }, { deleted: true });
...
router.post("/undeletebookappointment", async function (req, res, next) {
  ...
  try {
    await PaediatricianBooking.updateOne({ _id: req.query.id }, { deleted: false });
