...
router.post('/signin', async function(req, res, next) {
    ...
    try
    {
        ...
        res.status(200).send({status: 'OK', token: authToken, roles: ["dentistadmin"] }) 
    }
    catch(err)
    {
        console.log(err)
...
router.post('/checktoken', async function(req, res, next) {
    try{
        ...
        res.status(200).send({status:'OK', userId: "admin", forename: "admin", surname: "", roles:  ["dentistadmin"] , lastLoginTimeStamp: new Date()})
    }
    catch(err)
    {
        console.log(err)
