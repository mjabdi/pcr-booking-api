...
const {sendConfirmationEmail, sendRegFormEmail, sendBloodResultEmail, sendRefundNotificationEmail} = require('./email-service');
...
router.get('/getarchivedbloodreports', async function(req, res, next) {
    ...
    try{
        ...
        const result = await BloodReport.find({status: {$ne: "new"} , seen : {$eq : true}, emailSent: {$ne: true}}).sort({testDate:-1,timeStamp:-1}).exec();
...
router.get('/getnewmatchedbloodreports', async function(req, res, next) {
    ...
    try{
        ...
        const result = await BloodReport.find({status: {$ne: "new"} , email: {$ne: null}, seen : {$ne : true}, emailSent: {$ne: true}}).sort({testDate:-1, timeStamp:-1}).exec();
...
router.get('/getarchivedmatchedbloodreports', async function(req, res, next) {
    ...
    try{
        ...
        // const result = await BloodReport.find({status: "matched", seen : {$eq : true}}).sort({testDate:-1,timeStamp:-1}).exec();
        const result = await BloodReport.find({status: {$ne: "new"} , email: {$ne: null}, seen : {$eq : true}, emailSent: {$ne: true}}).sort({testDate:-1,timeStamp:-1}).exec();
...
router.get('/getnewunmatchedbloodreports', async function(req, res, next) {
    ...
    try{
        ...
        const result = await BloodReport.find({status: {$ne: "new"} , email: {$eq: null}, seen : {$ne : true}, emailSent: {$ne: true}}).sort({testDate:-1, timeStamp:-1}).exec();
...
router.get('/getarchivedunmatchedbloodreports', async function(req, res, next) {
    ...
    try{
        ...
        const result = await BloodReport.find({status: {$ne: "new"} , email: {$eq: null}, seen : {$eq : true}, emailSent: {$ne: true}}).sort({testDate:-1,timeStamp:-1}).exec();
...
router.get('/getsentbloodreports', async function(req, res, next) {
    ...
    try{
        ...
        // const result = await BloodReport.find({status: "unmatched", seen : {$eq : true}}).sort({testDate:-1,timeStamp:-1}).exec();
        const result = await BloodReport.find({status: {$ne: "new"} , emailSent: {$eq: true}}).sort({testDate:-1,timeStamp:-1}).exec();
...
router.get('/getnewbloodresultscount', async function(req, res, next) {
    ...
    try{
        ...
        // const result = await BloodReport.find({status: "unmatched", seen : {$eq : true}}).sort({testDate:-1,timeStamp:-1}).exec();
        const result = await BloodReport.countDocuments({status: {$ne: "new"} , emailSent: {$ne: true}, seen: {$ne : true}});
...
router.get('/getlivebookings', async function(req, res, next) {
    ...
    try{
         const result = await BloodBooking.find({bookingDate : {$gt : "2022-02-19"}, status: "patient_attended", deleted : {$ne : true }}).sort({bookingDate: 1}).exec();
         res.status(200).send(result);
    }
    catch(err)
    {
...
router.get('/getcompletedbookings', async function (req, res, next) {
    ...
    try {
        const limit = parseInt(req.query.limit) || DEFAULT_LIMIT
        const result = await BloodBooking.find({ status: "report_sent", deleted: { $ne: true }}).sort({ bookingDate: -1, bookingTimeNormalized: -1 }).limit(limit).exec();
        res.status(200).send(result);
    }
    catch (err) {
        res.status(500).send({ status: 'FAILED', error: err.message });
...
router.post('/addnewbooking', async function(req, res, next) {
    try{
        ...
        let {fullname, bookingDate, bookingTime, phone, email, notes, packageName, birthDate, indivisualTests, gender, smsPush} = req.body
  
        const payload =  {fullname, bookingDate, bookingTime, phone, email, notes, packageName, birthDate, indivisualTests, gender, smsPush}
...
router.post('/changetocompleted', async function(req, res, next) {
    ...
    try{
        ...
        await BloodBooking.updateOne({_id : req.query.id}, {status : 'report_sent', samplingTimeStamp: new Date()});
